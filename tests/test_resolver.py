...
from jsonseeder.exceptions import UnresolvedReferencesError
...
def test_resolver_bad_reference(model, resolver_populated, session):
    ...
    with pytest.raises(UnresolvedReferencesError):
        resolver_populated.load_entities_from_json_dict(AIRPORT_COUNTRY_REFERENCE_FIELD_OK, commit=True)
